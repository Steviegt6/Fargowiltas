...
namespace Fargowiltas.Items.Summons.SwarmSummons.Thorium
{
    ...
    public class OverloadViscount : ModItem
    {
        ...
        public override bool CanUseItem(Player player) => !Fargowiltas.SwarmActive && (player.ZoneDirtLayerHeight || player.ZoneRockLayerHeight);
        ...
        public override bool UseItem(Player player)
        {
            ...
            {
                int boss = NPC.NewNPC((int)player.position.X + Main.rand.Next(-1000, 1000), (int)player.position.Y + Main.rand.Next(-1000, -400), Fargowiltas.LoadedMods["Thorium"].NPCType("Viscount"));
                Main.npc[boss].GetGlobalNPC<FargoGlobalNPC>().SwarmActive = true;
            }

            if (Main.netMode == NetmodeID.Server)
            {
                ChatHelper.BroadcastChatMessage(NetworkText.FromLiteral("The caverns are dripping with blood!"), new Color(175, 75, 255));
            }
            else
            {
